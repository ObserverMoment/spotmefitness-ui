...
class WorkoutPlanFiltersBloc extends ChangeNotifier {
  ...
  WorkoutPlanFiltersBloc() {
    ...
    final json = workoutPlanFiltersFromDevice == null
        ? null
        : DataUtils.convertToJsonMap(workoutPlanFiltersFromDevice);
...
class WorkoutPlanFilters {
  ...
  bool areSameFilters(WorkoutPlanFilters o) {
    ...
    return e.equals(workoutGoals, o.workoutGoals) &&
  ...
  List<WorkoutPlan> filter(List<WorkoutPlan> plans) {
    Iterable<WorkoutPlan> byLengthWeeks = lengthWeeks == null
        ? [...plans]
        : plans.where((p) => p.lengthWeeks == lengthWeeks);

    Iterable<WorkoutPlan> byDaysPerWeek = daysPerWeek == null
        ? byLengthWeeks
        : plans.where((p) => p.daysPerWeek == daysPerWeek);

    Iterable<WorkoutPlan> byBodyweightOnly = bodyweightOnly == null
        ? byDaysPerWeek
    ...
    Iterable<WorkoutPlan> byDifficultyLevel = difficultyLevel == null
        ? byBodyweightOnly
        : plans.where((p) => _planContainsDifficultyLevel(p));

    /// At least one of the plans workouts must have a specified goal.
    Iterable<WorkoutPlan> byWorkoutGoals = workoutGoals.isEmpty
        ? byDifficultyLevel
