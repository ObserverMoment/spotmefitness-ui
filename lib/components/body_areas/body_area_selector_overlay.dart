...
class BodyAreaSelectorOverlay extends StatelessWidget {
  ...
  Future<List<PathBodyAreaMap>> _getAllSelectablePaths(
    ...
    List<List<PathBodyAreaMap>> _pathsNested = await Future.wait(bodyAreas.map(
        (bodyArea) => _parsePathsFromSvgAsset(bodyArea,
  ...
  Widget build(BuildContext context) {
    List<BodyArea> bodyAreasToDraw = allBodyAreas
        .where((ba) =>
...
class PathClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    _matrix4.scale(size.width / kBodyAreaSelector_svg_width,
        size.height / kBodyAreaSelector_svg_height);
